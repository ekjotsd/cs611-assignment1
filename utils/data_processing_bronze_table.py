...
def process_bronze_clickstream_table(snapshot_date_str, bronze_clickstream_directory, spark):
    """
    Process clickstream data for bronze layer
    """
    # prepare arguments
    ...
    print(f'{snapshot_date_str} clickstream row count:', df.count())
...
def process_bronze_attributes_table(snapshot_date_str, bronze_attributes_directory, spark):
    """
    Process customer attributes data for bronze layer
    """
    # prepare arguments
    snapshot_date = datetime.strptime(snapshot_date_str, "%Y-%m-%d")
    ...
    # load data - IRL ingest from back end source system
    df = spark.read.csv(csv_file_path, header=True, inferSchema=True).filter(col('snapshot_date') == snapshot_date_str)
    print(f'{snapshot_date_str} attributes row count:', df.count())
...
def process_bronze_financials_table(snapshot_date_str, bronze_financials_directory, spark):
    """
    Process customer financial data for bronze layer
    """
    # prepare arguments
    snapshot_date = datetime.strptime(snapshot_date_str, "%Y-%m-%d")
    ...
    # load data - IRL ingest from back end source system
    df = spark.read.csv(csv_file_path, header=True, inferSchema=True).filter(col('snapshot_date') == snapshot_date_str)
    print(f'{snapshot_date_str} financials row count:', df.count())
...
def process_bronze_lms_table(snapshot_date_str, bronze_lms_directory, spark):
    """
    Process LMS loan daily data for bronze layer (reusing from Lab 2)
    """
    # prepare arguments
    snapshot_date = datetime.strptime(snapshot_date_str, "%Y-%m-%d")
    ...
    # load data - IRL ingest from back end source system
    df = spark.read.csv(csv_file_path, header=True, inferSchema=True).filter(col('snapshot_date') == snapshot_date_str)
    print(f'{snapshot_date_str} LMS row count:', df.count())
